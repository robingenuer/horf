...
comput_bias <- function(
  ...
  for (k in nbleaves) {
    ...
    for.bias[[m]] <- parallel::mclapply(1:nfor, function(j) {
      forest <- forest_tilde(k = k, xdim = xdim, freg.name = freg.name,
                             ntree = ntree, nbobs = nbobs, mtry = mtry)
    ...
    if (var_estim) {
      ...
      for (i in 1:nfor) {
        allCrossedSquaredErrors[1:nbobs_test + (i-1)*nbobs_test,
                                1:nbobs_test + (i-1)*nbobs_test] <- NA
      }
  ...
  colnames(for.bias.res) <- c("nbleaves", "mean", "sd")
